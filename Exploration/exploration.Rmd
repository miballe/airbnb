...
I have created a script to read in all the data files as well as a script which takes 2 features and compares the rate of feature "output" grouped by feature "feature"
...
```{r, results='hold'}
invisible(compare_plot(output = "country_destination"))
```
...
test$feature_value <- as.character(test$feature_value)
ggplot(test, aes(x = reorder(feature_value, as.numeric(feature)), 
                 y = NDF_rate_relative, colour = feature)) + 
    geom_point() +

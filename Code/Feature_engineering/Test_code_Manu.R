...
# load data, data to try: users_FE_1/2/3/4
path <- 'C:/Users/Manuel/Desktop/Southampton/Data_Mining/airbnb_project/'
df_all_combined = readRDS(paste0(path,'users_FE_2.Rda'))
df_train = filter(df_all_combined, dataset == "train")
df_test = filter(df_all_combined, dataset == "test")
labels = df_train['country_destination']
df_train = select(df_train, -c(country_destination, dataset))
df_test = select(df_test, -c(country_destination, dataset))
